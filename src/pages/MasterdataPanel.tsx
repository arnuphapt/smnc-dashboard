...
import { LookupsTab } from './masterdata/LookupsTab'
...
export const MasterdataPanel: React.FC = () => {
  ...
  const handleAddLookup = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const nextSortOrder = options.filter((o) => o.category === lookupCategory).length
      const { error } = await supabase
        .from('lookup_options')
        .insert({
      ...
      <ContentPanel>
        {(() => {
          const getActiveTab = () => {
            ...
            if (path.startsWith('/master/lookups')) return 'lookups'
          ...
          const itemCategory = location.pathname.startsWith('/master/items/')
            ? location.pathname.split('/master/items/')[1]?.split('/')[0] || ''
            : ''
          const lookupPathCategory = location.pathname.startsWith('/master/lookups/')
            ? location.pathname.split('/master/lookups/')[1]?.split('/')[0] || ''
            : ''
