      ...
      <DialogContent className="max-w-lg max-h-[85vh] flex flex-col p-0 gap-0 overflow-hidden bg-white border border-[#E2E8F0] rounded-3xl shadow-2xl">
        ...
        <div className="px-6 py-4 overflow-y-auto flex-1 min-h-0 space-y-2">
          {subAttach.length === 0 ? (
            ...
            subAttach.map((at) => {
              const isSendBack = at.file_name?.includes('[เอกสารส่งกลับแก้ไข]')
              const isRevision = at.file_name?.includes('[ฉบับแก้ไข]')
              const isEval = at.file_name?.includes('[ประเมิน]')

              let cardStyle = 'bg-slate-50 border-slate-200 text-slate-700 hover:bg-slate-100 hover:border-slate-300'
              let badge: React.ReactNode = null

              if (isSendBack) {
                cardStyle = 'bg-amber-50/80 border-amber-200 text-amber-900 hover:bg-amber-100'
                badge = <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-amber-200/80 text-amber-900 shrink-0">เอกสารส่งกลับแก้ไข</span>
              } else if (isRevision) {
                cardStyle = 'bg-emerald-50/80 border-emerald-200 text-emerald-900 hover:bg-emerald-100'
                badge = <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-emerald-200/80 text-emerald-900 shrink-0">เล่มฉบับแก้ไข</span>
              } else if (isEval) {
                cardStyle = 'bg-purple-50/80 border-purple-200 text-purple-900 hover:bg-purple-100'
                badge = <span className="text-[10px] font-black px-2 py-0.5 rounded-full bg-purple-200/80 text-purple-900 shrink-0">ผลประเมิน</span>
              }
              ...
              return (
                <button
                  ...
                  className={`w-full flex items-center justify-between gap-3 px-3.5 py-2.5 rounded-2xl text-xs font-bold border transition-colors cursor-pointer shadow-xs text-left ${cardStyle}`}
                >
                  <div className="flex items-center gap-2.5 min-w-0 flex-1">
                    <PdfIcon className="w-4 h-4 shrink-0 text-slate-500" />
